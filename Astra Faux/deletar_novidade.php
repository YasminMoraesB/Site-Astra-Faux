...
?>
    ...
    <?php
        ...
        if (!empty($_GET['id'])) {
            ...
            $consulta = $mysqli->prepare("SELECT * FROM novidade WHERE id = ?");
            ...
            if ($resultado->num_rows > 0) {
                ...
                $exclusao = $mysqli->prepare("DELETE FROM novidade WHERE id = ?");
                $exclusao->bind_param("i", $id);
                $exclusao->execute();

                //Confirmação e Redirecionamento após a exclusão
                echo "<script>alert('Exclusão feita com sucesso!');";
                echo "javascript:window.location='consultar_novidade.php';</script>";
